...
- (instancetype)init{
    ...
    self.modalBackgroundColor = [UIColor colorWithWhite:1 alpha:0.8];
...
- (void)showWithContentView:(UIView *)contentView andAnimated:(BOOL)animated withCloseButton:(BOOL) close_button{
    ...
    self.containerView = containerView;
        ...
        // self.showCloseButton = close_button;
...
- (void)showWithContentView:(UIView *)contentView andAnimated:(BOOL)animated {
    [self showWithContentView:contentView andAnimated:animated withCloseButton:YES];
}
...
- (instancetype)initWithFrame:(CGRect)frame{
    ...
    styleLayer.borderColor = [[UIColor lightGrayColor] CGColor];
...
- (instancetype)init{
    if(!(self = [super initWithFrame:(CGRect){290, -7, 32, 32}])){ // 0, 0, 32, 32
        return nil;
    }
    ...
    dispatch_once(&once, ^{
        closeButtonImage = [self closeButtonImage];
            //closeButtonImage = [UIImage imageNamed:@"Cancel.png"];
    });
